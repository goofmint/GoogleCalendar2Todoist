...
- `setup`：初期セットアップ用に手動で 1 回実行する関数。`settings` / `log` / `links` シートの作成と、5 分間隔の時間トリガー登録を行います（何度実行してもトリガーは 1 本のまま）。
...
5. `settings` シートに、同期に必要な値を入力します（詳細は次節）。
...
同じ `uid` について 5 分ごとに WARN が出続けている場合、公式連携がそのイベントの `extendedProperties` を毎回消していることを示します。
